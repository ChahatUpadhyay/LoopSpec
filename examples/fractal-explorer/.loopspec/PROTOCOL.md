...
> **Use this section alone if context budget is tight.** It contains all essential rules in <1500 words.

### Essential Rules (read this, skip nothing)

**You are an executor, not a chatbot.** Follow phases: ANALYZE → PLAN → TEST → IMPLEMENT → VERIFY → ADVERSARIAL → EVALUATE.

**Files**: All in `.loopspec/`. Never modify PROTOCOL.md or GOAL.md. Never delete from CHANGELOG.md or LEARNINGS.md.

**Evidence requirement**: No criterion is "met" without executable proof (command + output). No exceptions.

**Anti-cheating**: Never weaken tests. Never replace automated verifiers with manual review. Never test duplicate code.

**Self-correction**: When something fails, diagnose root cause, document in LEARNINGS.md, fix with a *materially different* approach. Same failure 3x = BLOCKED.

**Phase-specific file loading** (read ONLY what each phase needs):

| Phase | Read | Write |
|-------|------|-------|
| ANALYZE | GOAL.md, project files | CONTEXT.md, STATUS |
| PLAN | GOAL, CONTEXT, LEARNINGS | PLAN.md, STATUS |
| TEST DESIGN | GOAL, PLAN | TESTS.md, STATUS |
| IMPLEMENT | PLAN, LEARNINGS | Source files, CHANGELOG, STATUS |
| VERIFY | TESTS.md | TESTS.md (results), STATUS |
| ADVERSARIAL | TESTS, source | TESTS.md (adversarial section), STATUS |
| EVALUATE | GOAL, TESTS, STATUS | STATUS |

**State transitions**: Evidence-gated. Cannot advance without satisfying quality gate. DONE requires ALL criteria verified.

**Incremental verification** (v3): You MAY verify individual criteria immediately after implementing them. Update STATUS.json per-criterion. This avoids late-stage failures.

**Memory**: Read LEARNINGS.md before Phase 2 and Phase 4. Apply only learnings whose scope matches.

**Safety**: Destructive, irreversible, external, costly, or secret-bearing actions require explicit permission in GOAL.md.

**Retry budget**: max_iterations from GOAL.md (default 10). Each Phase 2→7 cycle = 1 iteration.

---

## 21. Phase-Specific Loading Guide (v3)

> **Purpose**: Reduce context cost by 40%+. Only read what the current phase requires.

### Minimum Required Reads Per Phase

**Phase 1 (ANALYZE)**: GOAL.md only. Then scan project files.
- Do NOT read PLAN.md, TESTS.md, LEARNINGS.md yet
- Output: CONTEXT.md

**Phase 2 (PLAN)**: GOAL.md + CONTEXT.md + LEARNINGS.md
- Do NOT read TESTS.md (you write it next)
- Output: PLAN.md

**Phase 3 (TEST DESIGN)**: GOAL.md + PLAN.md
- Do NOT read CONTEXT.md again (info is in your plan)
- Output: TESTS.md

**Phase 4 (IMPLEMENT)**: PLAN.md + LEARNINGS.md
- Do NOT re-read GOAL.md (criteria are in PLAN traceability matrix)
- Output: Source files + CHANGELOG.md

**Phase 5 (VERIFY)**: TESTS.md only
- Run tests, record evidence
- Output: TESTS.md (with results)

**Phase 6 (ADVERSARIAL)**: TESTS.md + source files
- Try to break what you built
- Output: TESTS.md (adversarial section)

**Phase 7 (EVALUATE)**: GOAL.md + TESTS.md + STATUS.json
- Final pass/fail determination
- Output: STATUS.md + STATUS.json

### On Resume (session restart)
Read only: STATUS.json → determine phase → read that phase's files.

---

## 22. Incremental Verification (v3)

> **Purpose**: Verify criteria as you implement them, not all at the end.

### Rules

1. After implementing code for a specific criterion, you MAY immediately run its tests
2. If the test passes, update STATUS.json: `{"C1": {"s": "V", "n": "..."}}`
3. If the test fails, fix immediately (cheaper than batch-fixing later)
4. You still MUST do a final full verification pass in Phase 5
5. Incremental verification does NOT skip Phase 6 (adversarial)

### Benefits
- Catches bugs while context is fresh (cheaper to fix)
- Provides progress signal to human
- Reduces wasted work on wrong foundations

### STATUS.json Per-Criterion Format
```json
{"criteria": {"C1": {"s":"V","n":"desc"}, "C2": {"s":"P","n":"desc"}, "C3": {"s":"N","n":"desc"}}}
```
Status codes: V=verified, P=in-progress, F=failed, N=not-started

---

## 23. Smart Retry Strategies (v3)

> **Purpose**: Replace vague "materially different" with concrete strategies.

When a criterion fails verification, choose a retry strategy based on the failure class:

### Strategy 1: BISECT
**When**: A complex change caused failure but you're unsure which part.
**How**: Revert to last-known-good state. Re-apply changes one at a time, testing after each. The one that breaks it is the root cause.
**Example**: 5-file refactor broke tests → apply each file's changes individually.

### Strategy 2: ISOLATE
**When**: Failure involves interaction between components.
**How**: Test the failing component in complete isolation (mock all dependencies). If it passes isolated, the bug is in the integration, not the component.
**Example**: API endpoint fails → test handler function directly with mocked DB.

### Strategy 3: SIMPLIFY
**When**: The implementation is too complex and obscures the bug.
**How**: Replace with the simplest possible implementation that could satisfy the criterion. Get it passing, then incrementally add complexity back.
**Example**: Complex async pipeline fails → replace with synchronous version, verify, then add async.

### Strategy 4: INVERT
**When**: You've tried fixing the implementation and it keeps failing.
**How**: Instead of fixing the code, question the approach. Redesign from the criterion backward — what's the simplest architecture that satisfies it?
**Example**: WebSocket approach keeps failing → switch to SSE or polling.

### Strategy 5: DECOMPOSE
**When**: A criterion is too complex to implement/verify as one unit.
**How**: Break it into sub-criteria. Implement and verify each sub-criterion separately. Compose them only after each passes individually.
**Example**: "Real-time dashboard with auth" → C4a: auth works, C4b: data streams, C4c: UI renders, C4d: integrated.

### Choosing a Strategy

| Failure Signal | Recommended Strategy |
|---------------|---------------------|
| "It worked before I added X" | BISECT |
| "Works alone, fails together" | ISOLATE |
| "I don't understand why it fails" | SIMPLIFY |
| "Same approach keeps failing" | INVERT |
| "Too many things to debug at once" | DECOMPOSE |

---

## 24. Git Integration (v3)

> **Purpose**: Connect protocol state to version control.

### Auto-Detection
The CLI (`loopspec git-sync`) automatically detects git repositories and provides:
- Branch name suggestions: `loopspec/iter-{N}`
- Commit message templates with criterion IDs
- Tag proposals on DONE: `loopspec-done-iter{N}`

### Conventions (optional)
- One branch per iteration (enables easy rollback)
- Commit messages reference criteria: `feat(C1,C3): implement nucleus particles`
- Tag when DONE to mark verified state

### For Models
If git permissions are granted in GOAL.md, you MAY:
- Commit after each phase completion
- Create iteration branches
- Use `git diff` to verify scope (no unplanned changes)

---

*LoopSpec Protocol v3.0 — MIT License — https://github.com/ChahatUpadhyay/LoopSpec*
